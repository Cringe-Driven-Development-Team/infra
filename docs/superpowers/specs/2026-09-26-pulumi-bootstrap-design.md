...
- Основной стек хранит стейт в том же бакете под префиксом `main/`, теми же ключами.
...
- `initProjectS3`: успех — 2xx или 400 с признаком «уже инициализирован» в теле; любой другой
  ответ — ошибка с кодом и первыми 300 символами тела. Признак берётся из реального ответа Selectel
  при повторном `up` и фиксируется в тесте.
...
1. `bun test` без сети:
   - `selectel-s3.ts` с подменяемой HTTP-функцией: 201, 204, 400 «уже», 400 другое, 500, сетевая

...
- 2026-09-26, задача #7: проект переименован в `infra-shared` (логическое имя ресурса прежнее) и
  хранит также DNS-зону `cellestial.ru.` (`DomainsZoneV2`, `protect`) — долгоживущие ресурсы в одном
  проекте, прод отдельно. Выходы `dnsZone`, `dnsProjectId` — для `infra:dnsProjectId` основного стека.
